...
> Local reading mirror of [`docs/api-contract-v1.md`](docs/api-contract-v1.md). The tracked
> `docs/` directory was retired after backend/frontend 1.1.0; edit the local canonical copy first
> and keep this mirror aligned while it remains useful.
...
## 6. Email and password authentication (feature 1.1.3)
...
handoff `code` the OAuth flow already returns (see `EmailAuthHandoffResponse` below), which the BFF
exchanges for the opaque `lo_sid` browser session. Every endpoint lives under `/v1/auth/email`.
...
- The 8-digit code is generated with a CSPRNG, is valid for **10 minutes**, is accepted **at most
  once**, and is **exhausted after five wrong entries**. The attempt check and constant-time
  comparison run under a per-subject row lock; a valid code is then consumed in the *same*
  transaction that creates the account (signup) or replaces the password and revokes sessions
  (reset). So the change is atomic — a hashing or database failure never burns a valid code — and
  the five-attempt cap and single-use guarantee hold under concurrent requests.
...
- A password rejected by the local/HIBP safety check returns `422 PASSWORD_COMPROMISED` without
  consuming the valid OTP, so the user can choose another password and retry.
